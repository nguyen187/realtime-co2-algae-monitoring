...
schema_raman_silver = StructType([
StructField("Time stream", TimestampType()),
StructField("Cust", StringType()),
StructField("Project_ID", StringType()),
StructField("BatchID", LongType()),
...
def createDeltaTable(database_name,table_name, table_schema, table_location):
    ...
    df.write.format("delta").mode("overwrite").option("delta.columnMapping.mode", "name").option("path", table_location).option("overwriteSchema", "true").saveAsTable(table_name)
...
database_name = 'stream_raman'
delta_table_name = 'bronze_raman'
delta_location =f'/mnt/datalakehouse/bronze/bronze_stream_fd/{delta_table_name}.delta'
createDeltaTable(database_name,delta_table_name,schema_raman_raw, delta_location)

# COMMAND ----------

database_name = 'stream_raman'
delta_table_name = 'silver_raman'
delta_location =f'/mnt/datalakehouse/silver/silver_stream_fd/{delta_table_name}.delta'
createDeltaTable(database_name,delta_table_name,schema_raman_silver, delta_location)

# COMMAND ----------

database_name = 'stream_raman'
delta_table_name = 'gold_raman'
delta_location =f'/mnt/datalakehouse/gold/gold_stream_fd/{delta_table_name}.delta'
createDeltaTable(database_name,delta_table_name,schema_raman_gold, delta_location)

# COMMAND ----------

# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS default.silver_raman
# MAGIC
